...
def atualizar_form(i):
    with con:
        cur = con.cursor()
        query = "UPDATE AcervoBiblioteca SET material=?, codigo_titulo=?, titulo=?, numero_de_chamada=?, data_publicacao=?, autor=?, forma_aquisicao=?, data_aquisicao=?, fornecedor=?, preco=?"
        cur.execute(query, i)
